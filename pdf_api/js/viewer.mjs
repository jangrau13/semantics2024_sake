...
async function handleUpdatePDFAfterKGAdd(data, document) {
  ...
  savePDFButton.click();
  wiserEventBus.emit('knowledgeConfirmation', {
    updatedList
  });
}
...
const pdfjsVersion = "4.3.96";
const pdfjsBuild = "9ee09ccee";
const AppConstants = {
...
class Modal {
  constructor() {
    ...
    this.initStore().then(success => {
      ...
      web_WiserEventBus.on('downloadKnowledge', () => {
    ...
    window.myAtomicWorker.onmessage = async e => {
      switch (e.data.type) {
        case "pong":
          ...
          web_WiserEventBus.emit(magicWord, {
            content,
            infoToLookFor,
            potentialSubject,
            laScript
